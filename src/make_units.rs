...
macro_rules! make_units { ($System:ident, $allowed_root:ident; base { $($Type:ident, $constant:ident, $print_as:ident;)* } derived {$($Derived:ident($derived_constant: ident) = $e: expr;    )*} ) => (
    ...
    _make_types!($System, $allowed_root | $($Type),* | $($Type),* |);
    // fixme: when _make_types is fixed, use following line to call:
    // _make_types!($System, $allowed_root | $($Type),* | $($Type),*);
...
macro_rules! _make_types {
    ...
    ($System: ident, $($Nums: ident),+ | $($Types: ident),+ | $DeadType: ident, $($Others: ident),* | $($New: ident),*) => (
        _make_types!($System, $($Nums),+, Zero | $($Types),+ | $($Others),* | $DeadType $(, $New)*);
        );
    ...
    ($System: ident, $First: ident, $($Nums: ident),* | $($Types: ident),+ | $Type: ident | $($New: ident),+) => (
        ...
        _make_types!($System, $($Nums),*, $First | $Type, $($New),* );
        );
    ...
    ($System: ident, $First: ident, $($Nums: ident),* | $Type: ident, $($Types: ident),+) => (
        ...
        _make_types!($System, $($Nums),*, $First | $($Types),*);
        );
...
// // we call this with _make_types!(Num | Types | Types)
// #[doc_hidden]
// #[macro_export]
// macro_rules! _make_types {
//     // this first arm filters out the end Types into Zeros with Num, so we go from say
...
// macro_rules! _make_derived_type {
//     ($D: ident, $e: expr) => ();
// }

// macro_rules! _convert_expression {
//     ($a: ident * $b: expr) => ($a as AddDim<_convert_expression!{$b}>>::Output);
//     ($a: ident / $b: expr) => ($a as SubDim<_convert_expression!{$b}>>::Output);
//     ($a: ident) => ($a);
// }
